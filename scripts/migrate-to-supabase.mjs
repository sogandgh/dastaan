...
if (!SUPABASE_URL || !SERVICE_KEY) {
  console.error('Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY first. See README.md.');
  process.exit(1);
}
...
async function namespaceImages(dir) {
  ...
  files = files.filter(f => !f.includes('/') && f !== userId);
...
async function main() {
  ...
  console.log('\nDone. The old data/vocabulary.json and data/stories.json are untouched, safe to keep as a backup or delete once everything shows up in the app.');
}
